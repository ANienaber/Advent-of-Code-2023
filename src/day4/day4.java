...
with a better naming scheme. Tell you what: if you can help me with something quick, I'll let you borrow
my boat and you can go visit the gardener. I got all these scratchcards as a gift, but I can't figure
out what I've won."
...
public class day4 {
    ...
    private static void solvePart1() throws FileNotFoundException {
        List<String> lines = utils.readFile("src/day4/input4.txt");
        int sum = 0;

        for (String line : lines) {
            String numbersString = line.split(": ")[1];
            String[] winningNumbers = numbersString.split(" \\| ")[0].split(" ");
            String[] cardNumbers = numbersString.split(" \\| ")[1].split(" ");

            int points = calcPoints(winningNumbers, cardNumbers);
            sum += points;
            System.out.println(line + " has " + points + " points");
        }

        System.out.println("Sum: "+sum);
    }

    private static int calcPoints(String[] winningNumbers, String[] cardNumbers) {
        int points = 0;
    ...
    private static void solvePart2() throws FileNotFoundException {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            int winnings = calcWinnings(winningNumbers, cardNumbers);
            for (int j = i+1; j <= i + winnings && j < numCopies.length; j++) {
                numCopies[j] += numCopies[i];
            }
        }

        int totalCopies = getTotalCopies(numCopies);
        System.out.println("Total Copies are: "+totalCopies);

    }

    private static int calcWinnings(String[] winningNumbers, String[] cardNumbers) {
        int count = 0;
        for ( String winningNumber : winningNumbers) {
            for ( String cardNumber : cardNumbers ) {
                if((!cardNumber.equals("")) && (utils.getLongNumValue(winningNumber) == utils.getLongNumValue(cardNumber))){
    ...
    private static int getTotalCopies(int[] numCopies) {
        int sum = 0;
        for ( int num : numCopies ) {
            sum += num;
        }
        return sum;
    }
