...
* large island floating in the sky. You gently land in a fluffy pile of leaves. It's quite cold, but you
...
* with its ID number (like the 11 in Game 11: ...) followed by a semicolon-separated list of subsets of
...
* cubes and 4 red cubes; the second set is 1 red cube, 2 green cubes, and 6 blue cubes; the third set is
* only 2 green cubes.
...
* red cubes at once; similarly, game 4 would also have been impossible because the Elf showed you
...
* 13 green cubes, and 14 blue cubes. What is the sum of the IDs of those games? test
 */
        ...
        try(Scanner scanner = new Scanner(file)){
            ...
            while(scanner.hasNextLine()) {
                ...
                for (String turn : turns) {
                    ...
                    String[] entries = turn.split(", ");

                    for (String entry : entries) {
                        String[] ballcounts = entry.split(" ");
                        int num = getNumValue(ballcounts[0]);

                        switch (ballcounts[1]) {
                            case "red" -> possible = num <= numRed && possible;
                            case "green" -> possible = num <= numGreen && possible;
                            case "blue" -> possible = num <= numBlue && possible;
                            default -> {
                            }
                        }
                ...
                System.out.println("Index: "+idx+" possible: "+possible);
                if(possible){
                    sum += idx;
                }
                idx++;
